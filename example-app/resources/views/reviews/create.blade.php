        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <p class="content-center">Maak hier je Review. Geef jouw review een titel, waar jou review over gaat en selecteer daarna een product waar deze review over gaat!</p>
                    <br>
                    <form action="{{ route('reviews.create') }}" method="POST">
                        @csrf
                        <!--Review name -->
                        <div>
                            <x-input-label for="title" :value="__('Review Titel')" />
                            <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title')" required autofocus/>
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>
                        <!-- Review Description -->
                        <div>
                            <x-input-label for="content" :value="__('Review Inhoud')" />
                            <x-text-input id="content" class="block mt-1 w-full" type="text" name="content" :value="old('content')" required autofocus/>
                            <x-input-error :messages="$errors->get('content')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="product_id" :value="__('Selecteer Product')" />
